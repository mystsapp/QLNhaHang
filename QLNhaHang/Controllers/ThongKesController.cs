...
namespace QLNhaHang.Controllers
{
    public class ThongKesController : BaseController
    {
        ...
        public ThongKesController(IUnitOfWork unitOfWork)
        {
            ...
            ThongKeVM = new ThongKeViewModel()
            {
                DaInModels = new List<DaInModel>()
                {
                    ...
                    new DaInModel() { Id = "False", Name = "Chưa in" }
                },
        ...
        public ActionResult TheoNgay(ThongKeViewModel model)
        {
            ...
            xlSheet.Column(6).Width = 20;// tình trạng
            ...
            xlSheet.Cells[5, 6].Value = "Tình trạng";
            ...
            d = d.OrderBy(x => x.NhanVien.NoiLamViec).ToList();
            //du lieu
            int iRowIndex = 6;
            int idem = 1;
            ...
            if (d.Count != 0)
            {
                foreach (var vm in d)
                {
                    ...
                    TrSetCellBorder(xlSheet, iRowIndex, 1, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Center, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    ...
                    TrSetCellBorder(xlSheet, iRowIndex, 2, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    //xlSheet.Cells[iRowIndex, 2].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                    xlSheet.Cells[iRowIndex, 3].Value = vm.VanPhong.Name;
                    TrSetCellBorder(xlSheet, iRowIndex, 3, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Center, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    ...
                    TrSetCellBorder(xlSheet, iRowIndex, 4, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    //xlSheet.Cells[iRowIndex, 4].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                    xlSheet.Cells[iRowIndex, 5].Value = vm.ThanhTienHD;
                    TrSetCellBorder(xlSheet, iRowIndex, 5, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    //xlSheet.Cells[iRowIndex, 5].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                    // xlSheet.Cells[iRowIndex, 6].Value = vm.NhanVien.NoiLamViec;
                    ...
                    //xlSheet.Cells[iRowIndex, 6].Style.Font.Italic = true;

                    if (vm.DaIn == true)
                    {
                        xlSheet.Cells[iRowIndex, 6].Value = "Đã xuất HD";
                        TrSetCellBorder(xlSheet, iRowIndex, 6, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    }
                    else
                    {
                        xlSheet.Cells[iRowIndex, 6].Value = "Chưa xuất HD";
                        TrSetCellBorder(xlSheet, iRowIndex, 6, ExcelBorderStyle.Dotted, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
                    }
                    iRowIndex += 1;
                    idem += 1;
        ...
        public ActionResult TheoMon(int vanPhongId = 0, /*int khuVucId = 0,*/ string tuNgay = null, string denNgay = null)
        {
            ...
            ThongKeVM.ThucDons = _unitOfWork.thucDonRepository.GetAll().ToList().Where(x => x.LoaiThucDon.TenLoai.ToLower().Contains("buffet")).ToList();
            ...
            if (vanPhongId != 0)
            {
                ...
                ThongKeVM.ThucDons = ThongKeVM.ThucDons.Where(x => x.VanPhong == _unitOfWork.vanPhongRepository.GetById(vanPhongId).Name).ToList();
                if(ThongKeVM.ThucDons.Count == 0)
                {
                    ThongKeVM.ThucDons = new List<ThucDon>() { new ThucDon { Id = 0, TenMon = "Văn phòng này chưa có tiệc buffet." } };
                }
            }
        ...
        public ActionResult TheoMon(ThongKeViewModel model)
        {
            ...
            if (model.VanPhongId == 0)
            {
                ViewBag.searchFromDate = model.TuNgay;
                ViewBag.searchToDate = model.DenNgay;
                SetAlert("Bạn chưa chọn cơ sở.", "warning");
                return RedirectToAction(nameof(TheoMon), new { tuNgay = model.TuNgay, denNgay = model.TuNgay });
                
            }
            if (model.ThucDonId == 0)
            {
                SetAlert("Bạn chưa chọn tiệc buffet nào.", "warning");
                return RedirectToAction(nameof(TheoMon), new { tuNgay = model.TuNgay, denNgay = model.TuNgay, vanPhongId = model.VanPhongId });
            }
            var vanPhong = _unitOfWork.vanPhongRepository.GetById(model.VanPhongId);
            ...
            xlSheet.Column(4).Width = 20;// Ngày xuất
            xlSheet.Column(5).Width = 30;// Doanh số

            xlSheet.Cells[2, 1].Value = "BÁO CÁO DOANH THU THEO TIỆC BUFFET CƠ SỞ " + vanPhong.Name.ToUpper();
            xlSheet.Cells[2, 1].Style.Font.SetFromFont(new Font("Times New Roman", 16, FontStyle.Bold));
            xlSheet.Cells[2, 1, 2, 6].Merge = true;
            setCenterAligment(2, 1, 2, 6, xlSheet);
            // dinh dang tu ngay den ngay
            ...
            var d = _unitOfWork.thongKeRepository.ListHoaDonTheoTiecBuffet(model.VanPhongId, model.ThucDonId, model.TuNgay, model.DenNgay);// Session["daily"].ToString(), Session["khoi"].ToString());
            ...
            // Sum tổng tiền
            xlSheet.Cells[dong, 4].Value = "TC:";
            TrSetCellBorder(xlSheet, (6 + d.Count()), 4, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
            //DateTimeFormat(6, 4, 6 + d.Count(), 4, xlSheet);
            // DateTimeFormat(6, 4, 9, 4, xlSheet);
            //setCenterAligment(6, 4, 9, 4, xlSheet);
            xlSheet.Cells[dong, 5].Formula = "SUM(E6:E" + (5 + d.Count()) + ")";
            TrSetCellBorder(xlSheet, (6 + d.Count()), 5, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 11, FontStyle.Regular);
            //xlSheet.Cells[iRowIndex, 5].Style.Border.Right.Style = ExcelBorderStyle.Thin;

            xlSheet.Cells[6, 1, iRowIndex, 5].Style.Font.SetFromFont(new Font("Times New Roman", 11));
            ...
            // canh giua code chinhanh
            setCenterAligment(6, 3, 6 + d.Count(), 3, xlSheet);
            NumberFormat(6, 5, 6 + d.Count(), 5, xlSheet);
            // định dạng số cot tong cong
            //  NumberFormat(dong, 5, dong, 6, xlSheet);
            //setBorder(dong, 5, dong, 5, xlSheet);
            //setFontBold(dong, 5, dong, 5, 12, xlSheet);
            ...
            string sFilename = "DoanhThuBuffet_" + vanPhong.Name + "_" + System.DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss") + ".xlsx";
        ...
        public ActionResult BaoCaoTheoNhanVien(string tuNgay = null, string denNgay = null, int khuVucId = 0)
        {
            ...
            if (user.Role != "Admins")
            {
                if (user.Role == "Users")
                {
                    ...
                    ThongKeVM.NhanViens = new List<NhanVien>() { new NhanVien() { MaNV = user.MaNV, Username = user.Username } };
                }
                else
                {
        ...
        public ActionResult BaoCaoTheoNhanVien(ThongKeViewModel model)
        {
            ...
            if (nhanvien.NoiLamViec != "--None--")
            {
                return RedirectToAction(nameof(BaoCaoTheoNoiLamViecForAdmin), new { tuNgay = model.TuNgay, 
                                                                            denNgay = model.DenNgay, 
                                                                            vanPhongId = model.VanPhongId, 
                                                                            khuVucId = model.KhuVucId, 
                                                                            nhanVienId = model.NhanVienId});
            }
            //if(nhanvien.NoiLamViec == "Bếp")
            //{
            //    return RedirectToAction(nameof(BaoCaoTheoNoiLamViec), new { tuNgay = model.TuNgay, denNgay = model.DenNgay})
            //}
            var khuVuc = _unitOfWork.khuVucRepository.GetById(model.KhuVucId);
            ...
            xlSheet.Column(5).Width = 30;// Ngày bán
            xlSheet.Column(6).Width = 30;// Tên món
            xlSheet.Column(7).Width = 10;// Số lượng
            xlSheet.Column(8).Width = 10;// Đơn giá
            xlSheet.Column(9).Width = 20;// Thành tiền
            xlSheet.Column(10).Width = 30;// Ban
            ...
            xlSheet.Cells[5, 8].Value = "Đơn giá";
            xlSheet.Cells[5, 9].Value = "Thành tiền";
            xlSheet.Cells[5, 10].Value = "Bàn";
            ...
            if (hoaDons != null)
            {
                ...
                {
                    xlSheet.Cells[dong, 1, dong, 10].Merge = true;
                    ...
                    foreach (var item in vm.ChiTietHdViewModels)
                    {
                        ...
                        xlSheet.Cells[dong, 8].Value = item.DonGia;
                        ...
                        xlSheet.Cells[dong, 9].Value = item.ThanhTien;
                        ...
                        xlSheet.Cells[dong, 10].Value = item.TenBan;
                        TrSetCellBorder(xlSheet, dong, 10, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        ...
                        TrSetCellBorder(xlSheet, dong, 8, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        xlSheet.Cells[dong, 9].Value = item.TC;
        ...
        public ActionResult BaoCaoTheoNoiLamViec(string tuNgay = null, string denNgay = null, int khuVucId = 0)
        {
            ...
            {
                ViewBag.khuVuc = khuVucId;
                ThongKeVM.NhanViens = ThongKeVM.NhanViens.Where(x => x.KhuVucId == khuVucId && x.NoiLamViec == "Bếp").ToList();
                if (ThongKeVM.NhanViens.Count == 0)
                {
                    ThongKeVM.NhanViens = new List<NhanVien>() { new NhanVien { MaNV = "0", Username = "Chưa có nhân viên nào" } };
                }
            ...
            if (user.Role != "Admins")
            {
                ...
                else
                {
                    ...
                    foreach (var item in ThongKeVM.KhuVucs)
                    {
                        ThongKeVM.NhanViens.AddRange(_unitOfWork.nhanVienRepository.Find(x => x.KhuVucId == item.Id && x.NoiLamViec == "Bếp"));
                    }
                }
            }
            else
            {
                ...
                ThongKeVM.NhanViens = ThongKeVM.NhanViens.Where(x => x.NoiLamViec == "Bếp").ToList();
            }
        ...
        [HttpPost]
        public ActionResult BaoCaoTheoNoiLamViec(ThongKeViewModel model)
        {
            var user = (NhanVien)Session["UserSession"];
            ...
            //HauCanExport(model.TuNgay, model.DenNgay, model.KhuVucId, model.NhanVienId);
            ...
            if (nhanvien == null)
            {
                ...
                ViewBag.searchToDate = model.DenNgay;
                SetAlert("Chưa chọn nhân viên.", "warning");
                return RedirectToAction(nameof(BaoCaoTheoNoiLamViec));
            }
            ...
            xlSheet.Cells[2, 1].Value = "BÁO CÁO THEO NƠI LÀM VIỆC - " + nhanvien.HoTen.ToUpper();
            xlSheet.Cells[2, 1].Style.Font.SetFromFont(new Font("Times New Roman", 16, FontStyle.Bold));
            xlSheet.Cells[2, 1, 2, 10].Merge = true;
            setCenterAligment(2, 1, 2, 10, xlSheet);
            // dinh dang tu ngay den ngay
            ...
            {
                //if (DateTime.Parse(model.TuNgay) > DateTime.Parse(model.DenNgay))
                //{
                //    SetAlert("Từ ngày không được lớn đến ngày.", "warning");
                //    return RedirectToAction(nameof(BaoCaoTheoNoiLamViec));
                //}
                fromTo = "Từ ngày: " + model.TuNgay + " đến ngày: " + model.DenNgay;
            }
            ...
            var listPhache = new List<PhaChe>();
            if (nhanvien.NoiLamViec == "Bếp")
            {
                try
                {
                    listBep = _unitOfWork.thongKeRepository.ListHoaDonTheoBep(model.NhanVienId, model.TuNgay, model.DenNgay).ToList();
                }
                catch (Exception)
                {
                    
                    SetAlert("Lỗi định dạng ngày tháng.", "warning");
                    return RedirectToAction(nameof(BaoCaoTheoNoiLamViec), new { tuNgay = model.TuNgay , denNgay = model.DenNgay });
                }
                ...
                if (listBep.Count != 0)
                {
                    foreach (var item in listBep)
                    {
                        ...
                        xlSheet.Cells[dong, 2].Value = nhanvien.HoTen;
                        TrSetCellBorder(xlSheet, dong, 2, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        // xlSheet.Cells[dong, 2].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                        xlSheet.Cells[dong, 3].Value = item.ThucDon.VanPhong;
                        TrSetCellBorder(xlSheet, dong, 3, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        // xlSheet.Cells[dong, 3].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                        xlSheet.Cells[dong, 4].Value = nhanvien.KhuVuc.Name;
                        TrSetCellBorder(xlSheet, dong, 4, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        //xlSheet.Cells[dong, 4].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                        ...
                        xlSheet.Cells[dong, 6].Value = item.ThucDon.TenMon;
                        TrSetCellBorder(xlSheet, dong, 6, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        // xlSheet.Cells[dong, 6].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                        ...
                        xlSheet.Cells[dong, 8].Value = item.GiaTien;
                        TrSetCellBorder(xlSheet, dong, 8, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        // xlSheet.Cells[dong, 8].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                        ...
                        xlSheet.Cells[dong, 10].Value = item.Ban.TenBan;
                        TrSetCellBorder(xlSheet, dong, 10, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                        // xlSheet.Cells[dong, 10].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                ...
                xlSheet.Cells[dong, 8].Value = "TC:";
                TrSetCellBorder(xlSheet, dong, 8, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Right, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                xlSheet.Cells[dong, 9].Formula = "SUM(I6:I" + (dong - 1) + ")";
                //setBorder(dong, 8, dong, 9, xlSheet);
                TrSetCellBorder(xlSheet, dong, 9, ExcelBorderStyle.Thin, ExcelHorizontalAlignment.Left, Color.Silver, "Times New Roman", 10, FontStyle.Regular);
                //xlSheet.Cells[dong, 9].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                ...
                //setFontSize(6, 1, 6 + d.Count() + 2, 8, 11, xlSheet);
                // canh giua cot stt
                setCenterAligment(6, 1, 6 + listBep.Count() + 2, 1, xlSheet);
                // canh giua code chinhanh
                setCenterAligment(6, 3, 6 + listBep.Count() + 2, 3, xlSheet);
                // NumberFormat(6, 6, 6 + d.Count(), 6, xlSheet);
                // định dạng số cot, đơn giá, thành tiền tong cong
                // NumberFormat(6, 8, dong, 9, xlSheet);
            ...
            if (nhanvien.NoiLamViec == "Pha chế")
            {
                listPhache = _unitOfWork.thongKeRepository.ListHoaDonTheoPhaChe(model.NhanVienId, model.TuNgay, model.DenNgay).ToList();

                //du lieu
                //int iRowIndex = 6;
                int idem = 1;

                if (listPhache.Count != 0)
                {
                    foreach (var item in listPhache)
                    {
            ...
            string sFilename = "BaoCaoTheoNoiLamViecNhanVien_" + nhanvien.HoTen + "_" + System.DateTime.Now.ToString("dd_MM_yyyy_hh_mm_ss") + ".xlsx";
        ...
        public ActionResult BaoCaoTheoNoiLamViecForAdmin(string tuNgay = null, string denNgay = null, int VanPhongId = 0, int khuVucId = 0, string NhanVienId = null)
        {
            var user = (NhanVien)Session["UserSession"];
            ...
            ViewBag.vanPhong = VanPhongId;
            ViewBag.khuVuc = khuVucId;
            ViewBag.nhanVien = NhanVienId;

            //HauCanExport(model.TuNgay, model.DenNgay, model.KhuVucId, model.NhanVienId);

            
            var nhanvien = _unitOfWork.nhanVienRepository.GetByStringId(NhanVienId);
            if (nhanvien == null)
            {
                ViewBag.searchFromDate = tuNgay;
                ViewBag.searchToDate = denNgay;
            ...
            var khuVuc = _unitOfWork.khuVucRepository.GetById(khuVucId);
            ...
            if (nhanvien.NoiLamViec == "Bếp")
            {
                try
                {
                    listBep = _unitOfWork.thongKeRepository.ListHoaDonTheoBep(NhanVienId, tuNgay, denNgay).ToList();
                }
                catch (Exception)
                {
                    
                    SetAlert("Lỗi định dạng ngày tháng.", "warning");
                    return RedirectToAction(nameof(BaoCaoTheoNhanVien), new { tuNgay = tuNgay , denNgay = denNgay });
                }
            ...
            if (nhanvien.NoiLamViec == "Pha chế")
            {
                listPhache = _unitOfWork.thongKeRepository.ListHoaDonTheoPhaChe(NhanVienId, tuNgay, denNgay).ToList();
